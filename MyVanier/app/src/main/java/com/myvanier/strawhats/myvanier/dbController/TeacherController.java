...
public class TeacherController extends DatabaseController
{
    protected String DB_TABLE_NAME = "Teacher";
    ...
    /**
     * Selects entries from the database where name of the teacher matches the search String
     * @param search
     * @return a list with the selected entries
     */
    public List<Teacher> readAllFiltered(String search) {
        List<Teacher> teacherList = new ArrayList<>();

        sqLiteDatabase = this.databaseAccessHelper.openDatabase();
        try {
            Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM " + DB_TABLE_NAME + " WHERE teacher_name like ?", new String[]{"%"+search+"%"});
            cursor.moveToFirst();
